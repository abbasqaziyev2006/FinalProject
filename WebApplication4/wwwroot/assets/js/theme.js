...
(function () {
    ...
    UomoElements.InfiniteScroll = (function () {
        function InfiniteScroll(options) {
            this.selectors = {
                container: options.container || '#products-grid',
                loadingIndicator: options.loadingIndicator || '.infinite-scroll-loader',
                endMessage: options.endMessage || '.infinite-scroll-end'
            }

            this.apiEndpoint = options.apiEndpoint || '/Product/api/products/page/';
            this.triggerOffset = options.triggerOffset || 1000;
        ...
        InfiniteScroll.prototype = Object.assign({}, InfiniteScroll.prototype, {
            _init: function () {
            ...
            _handleScroll: function () {
                ...
                const triggerPoint = document.body.offsetHeight - this.triggerOffset;
            ...
            _loadMore: function () {
                ...
                fetch(`${this.apiEndpoint}${this.currentPage}`)
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(data => {
                        if (data.products && data.products.length > 0) {
                            this._appendItems(data.products);
            ...
            _appendItems: function (items) {
                const _this = this;
                items.forEach(item => {
                    const html = _this.onLoad ? _this.onLoad(item) : this._getDefaultHTML(item);
                    this.$container.insertAdjacentHTML('beforeend', html);
                });

                // Trigger any reinitialization needed for new content
                if (typeof UomoSections !== 'undefined' && UomoSections.SwiperSlideshow) {
                    new UomoSections.SwiperSlideshow();
                }
            },

            _getDefaultHTML: function (item) {
                return `
          <div class="product-card-wrapper">
            <div class="product-card mb-3 mb-md-4 mb-xxl-5">
              ...
              <div class="pc__info position-relative">
                <p class="pc__category">${item.category || 'Uncategorized'}</p>
                <h6 class="pc__title"><a href="/Product/Details/${item.id}">${item.name}</a></h6>
        ...
        `;
            ...
            _showEndMessage: function () {
                const $endMsg = document.querySelector(this.selectors.endMessage);
                if ($endMsg) {
    ...
    class Uomo {
        constructor() {
            ...
            // new UomoElements.Search();  // <-- BU SƏTRI COMMENT OUT EDİN
    ...
    // FIXED: Add null check for register button
    const registerElement = document.querySelector('.js-show-register');
    if (registerElement) {
